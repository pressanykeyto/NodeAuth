## Node Auth App
This is example for node login portal with user registration and with seperate views for admin and normal users
...
Add following line to ansible_playbook.yml after - name:  
```connection: local```  
...
```ansible remote_IP --become -m raw -a "apt install -y python-minimal python-simplejson" --ask-become-pass```  

To install and configure Postgres:  
```ansible-playbook ansible_playbook.yml --ask-become-pass```  

1. Clone repository
1. Run ```npm install```
1. Create new database in Postgres
1. Update ```setup.js``` with the postgres conncetion details
1. Run ```node setup.js``` and enter admin username and password
1. Run ```npm test``` to check if everything works
1. Enjoy & build great apps 😀 

### Users in DB:
No need to follow this step if ```node setup.js``` run successfully
...
### Base repo
https://github.com/DayOnePl/dos-server
